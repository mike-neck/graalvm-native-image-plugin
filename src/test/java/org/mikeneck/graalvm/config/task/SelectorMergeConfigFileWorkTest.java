...
class SelectorMergeConfigFileWorkTest {
    ...
    private static final MappingCandidate<FileInput, SelectableMergeableConfig<?>> olfResourceConfigParser = new FileInputToResourceConfigMappingForTest();
    private static final MappingCandidate<FileInput, SelectableMergeableConfig<?>> $20$3ResourceConfigParser = new FileInputToResourceConfig$20$3$MappingForTest();
    ...
    @SafeVarargs
    @NotNull
    private static MappingCandidates<FileInput, SelectableMergeableConfig<?>> parserUsing(@NotNull MappingCandidate<FileInput, SelectableMergeableConfig<?>>... parsers) {
    ...
    @DisplayName("parsing order:[ old resource-config -> 20-3 resource-config ], resources: [old resource-config, , 20-3 resource-config], , expects: exception thrown")
    @TestFactory
    Iterable<DynamicTest> expectedFailureCase1() throws IOException {
        ...
        return assertThrowsException(
                "expected IOException will be thrown",
                IOException.class,
                mergeConfigFileWork::run)
                .then("it won't have any message", exception ->
    ...
    @DisplayName("parsing order:[ 20-3 resource-config ->  old resource-config ], resources: [old resource-config, , 20-3 resource-config], , expects: exception thrown")
    @TestFactory
    ...
    interface IfExceptionThrownThen<E extends Throwable> {
        Iterable<DynamicTest> then(@NotNull String description, @NotNull Testing<? super E> testing) throws E;
    }

    private static <E extends Throwable> IfExceptionThrownThen<E> assertThrowsException(@NotNull String description, @NotNull Class<E> klass, @NotNull Executable throwingExecutable) {
        return (nextDesc, testing) -> {
            Throwable[] exceptions = new Throwable[1];
            DynamicTest first = DynamicTest.dynamicTest(description, () -> {
                E exception = assertThrows(klass, throwingExecutable);
                exceptions[0] = exception;
            });
            DynamicTest second = DynamicTest.dynamicTest(nextDesc, () -> {
