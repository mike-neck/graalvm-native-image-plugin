...
class ResourceCollectionTest {
  ...
  void expectedCaseWithSingleCandidate() throws IOException {
    List<String> inputs =
        Arrays.asList("expected-foo", "expected-bar", "expected-baz", "expected-qux-quux");
        ...
        MappingCandidates.forTest(
            input -> {
              Mapper<String, String> mapper =
                  string -> {
                    String newValue =
                        Arrays.stream(string.split("-")).skip(1).collect(Collectors.joining("-"));
                    if (newValue.isEmpty()) {
                      throw new IOException(String.format("%s does not have character[-]", string));
                    }
                    return newValue;
                  };
              return mapper.workOptional(input).map(first -> Outcome.forTest(first, mapper));
            });
  ...
  void hasFailureInputForSingleCandidate() {
    List<String> inputs =
        Arrays.asList("expected-foo", "expected-bar", "error_baz", "expected-qux-quux");
    ...
    IOException ioException =
        assertThrows(IOException.class, () -> processor.applyAll(mappingCandidates));
