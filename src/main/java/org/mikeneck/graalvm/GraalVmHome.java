...
class GraalVmHome {
  ...
  Optional<Path> graalVmUpdater() {
    return graalVmUpdaterCandidates().stream().filter(Files::exists).findFirst();
  }
  ...
  private List<Path> graalVmUpdaterCandidates() {
    return Arrays.asList(graalVmHome.resolve("bin/gu"), graalVmHome.resolve("bin/gu.cmd"));
  }

  @NotNull
  Optional<Path> nativeImage() {
    return nativeImageCandidates().stream().filter(Files::exists).findFirst();
  }

  private List<Path> nativeImageCandidates() {
    return Arrays.asList(
        graalVmHome.resolve("bin/native-image"), graalVmHome.resolve("bin/native-image.cmd"));
  }

  @NotNull
  Optional<Path> javaExecutable() {
    return javaExecutableCandidates().stream().filter(Files::exists).findFirst();
  }

  private List<Path> javaExecutableCandidates() {
    return Arrays.asList(graalVmHome.resolve("bin/java"), graalVmHome.resolve("bin/java.exe"));
  }
