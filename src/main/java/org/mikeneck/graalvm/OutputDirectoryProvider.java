...
class OutputDirectoryProvider extends DefaultProvider<File> {
  ...
  @SafeVarargs
  private static <T> Function<T, File> converter(
      Function<T, File> toFile, UnaryOperator<T>... validation) {
    UnaryOperator<T> operator =
        Arrays.stream(validation)
  ...
  static OutputDirectoryProvider ofFile(File nullable) {
    return new OutputDirectoryProvider(
        nullable, file -> file, outputDirectoryIsNotNull(), outputDirectoryIsNotExistingFile());
  }

  static OutputDirectoryProvider ofPath(Path nullable) {
    return new OutputDirectoryProvider(
        nullable, Path::toFile, outputDirectoryIsNotNull(), outputDirectoryIsNotExistingFilePath());
  }
}
