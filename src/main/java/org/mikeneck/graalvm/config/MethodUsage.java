...
public class MethodUsage implements Comparable<MethodUsage> {
  ...
  @NotNull public List<String> parameterTypes = new ArrayList<>();
  ...
  static MethodUsage of(@NotNull String name, @NotNull Class<?>... parameterTypes) {
    return new MethodUsage(
        name, Arrays.stream(parameterTypes).map(Class::getCanonicalName).toArray(String[]::new));
  }

  public static MethodUsage ofInit(@NotNull Class<?>... parameterTypes) {
    return new MethodUsage(
        "<init>",
        Arrays.stream(parameterTypes).map(Class::getCanonicalName).toArray(String[]::new));
  }
