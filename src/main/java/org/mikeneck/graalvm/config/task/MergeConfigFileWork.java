...
public interface MergeConfigFileWork<C extends MergeableConfig<C>> {
    ...
    static SelectorMergeConfigFileWorkFactory ofSelector() {
        ServiceLoader<SelectorMergeConfigFileWorkFactory> loader = ServiceLoader.load(SelectorMergeConfigFileWorkFactory.class);
        Iterator<SelectorMergeConfigFileWorkFactory> iterator = loader.iterator();
        if (iterator.hasNext()) {
            return iterator.next();
        } else {
            throw new IllegalStateException("Loader of " + SelectorMergeConfigFileWorkFactory.class + " not found");
        }
    }
}
