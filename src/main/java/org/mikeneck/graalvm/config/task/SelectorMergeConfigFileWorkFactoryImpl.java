...
public class SelectorMergeConfigFileWorkFactoryImpl implements SelectorMergeConfigFileWorkFactory {
  ...
  public FileInputsSelection resource(@NotNull String resourceName) {
    ...
    return files ->
        new SelectorMergeConfigFileWorkFactory.MappingCandidateSelection() {
          @Override
          public final <C extends SelectableMergeableConfig<C>>
              FileOutputSelection<C> resourceCandidateTypes(@NotNull TypeProvider... klasses) {
            return fileOutput -> {
              @SuppressWarnings("unchecked")
              List<MappingCandidate<FileInput, C>> candidates =
                  Arrays.stream(klasses)
                      .map(TypeProvider::get)
                      .<MappingCandidate<FileInput, C>>map(
                          klass ->
                              new MappingCandidateByObjectMapper<>((Class<C>) klass, objectMapper))
